...
test("deployment docs describe public admin and app domains", async () => {
  const [domain, publicDemo, adminAccess, adminPages, cloudflareAccess, appPlaceholder] = await Promise.all([
    ...
    readFile("docs/deployment/app-placeholder.md", "utf8")
  ]);
  for (const text of [domain, publicDemo, adminAccess, adminPages, cloudflareAccess, appPlaceholder]) {
  ...
  assert.ok(appPlaceholder.includes("ai-creator-os-app"));
  assert.ok(appPlaceholder.includes("build:app-placeholder"));
  assert.ok(appPlaceholder.includes("verify:app-access"));
});
