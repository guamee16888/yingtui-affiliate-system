...
console.log(`staging worker: ${config.env?.staging?.name || "(missing)"}`);
...
console.log(`APP_ENV: ${config.env?.staging?.vars?.APP_ENV || "(missing)"}`);
console.log(`APP_STORAGE_MODE: ${config.env?.staging?.vars?.APP_STORAGE_MODE || "(missing)"}`);
