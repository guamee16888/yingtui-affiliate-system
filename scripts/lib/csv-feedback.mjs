...
export function mapFeedbackCsv(text, { latest = null, feedback = null } = {}) {
  ...
  rows.slice(1).forEach((row, index) => {
    ...
    entries.push({
      ...
      sourceId: raw.sourceId || matchedFeedback?.sourceId || matchedTool?.sourceId || "",
      sourceName: raw.sourceName || matchedFeedback?.sourceName || matchedTool?.sourceName || "",
      sourceType: raw.sourceType || matchedFeedback?.sourceType || matchedTool?.sourceType || "",
      circle: raw.circle || matchedFeedback?.circle || matchedTool?.circle || "",
      candidateType: raw.candidateType || matchedFeedback?.candidateType || matchedTool?.candidateType || "",
      copyText,
      posted: true,
